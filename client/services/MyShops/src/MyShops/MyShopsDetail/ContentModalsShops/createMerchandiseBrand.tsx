...
import { createMerchandiseBrandAC } from "@packages/shared/src/store/MyShopsReducers/myShopsSlice";
...
const CreateMerchandiseBrand: React.FC<PropsType> = ({ shop, setModalActiveBrand }) => {
    const dispatch = useAppDispatch()
    const [inputValue, setInputValue] = useState('')

    const addBrand = () => {
        if(inputValue !== ''){
            dispatch(createMerchandiseBrandAC({
                merchandiseBrandName: inputValue,
                shopId: shop.shopId
            }))
            .then(data => {
                setInputValue('')
...
export default CreateMerchandiseBrand