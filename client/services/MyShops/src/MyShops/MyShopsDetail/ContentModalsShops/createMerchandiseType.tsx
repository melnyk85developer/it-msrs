...
import { createMerchandiseTypeAC } from "@packages/shared/src/store/MyShopsReducers/myShopsSlice";
...
const CreateMerchandiseType: React.FC<PropsType> = ({ shop, setModalActiveType }) => {
    const dispatch = useAppDispatch()
    const [inputValue, setInputValue] = useState('')

    const addType = () => {
        if(inputValue !== ''){
            dispatch(createMerchandiseTypeAC({
                merchandiseTypeName: inputValue,
                shopId: shop.shopId
            }))
            .then(data => {
                setInputValue('')
...
export default CreateMerchandiseType