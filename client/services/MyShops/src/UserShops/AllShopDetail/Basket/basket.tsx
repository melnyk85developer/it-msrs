...
import { getMyBasketAC, deleteToBasketMerchandiseAC } from '@packages/shared/src/store/MyShopsReducers/myShopsSlice';
import { BasketMerchandise, MyBasket, MyShopsType } from '@packages/shared/src/types/shopsTypes';
...
const Basket: React.FC<PropsType> = React.memo(({ 
    shop, 
    basket, 
    dispatch, 
    authorizedUser, 
    ...
    const userId = authorizedUser ? authorizedUser.id : undefined
    const shopId = shop ? shop.shopId : undefined
    const goods = basket.basketMerchandises;

    const deleteDevice = (deviceId: string) => {
        dispatch(deleteToBasketMerchandiseAC(deviceId));
    };
    ...
    const commoditys = useMemo(() => goods?.map((item: BasketMerchandise) =>
        <div key={item.basketMerchandiseId} className={classes.wrapItemBasketDevice}>
            <div className={classes.itemBasketDevice}>
                <img src={item.image ? `${API_URL}/` + item.image : defaultUserAvatar} />
                <div>
                    <div className={classes.name}>{item.merchandiseName}</div>
                ...
                <div className={classes.deleteBlock}>
                    <strong onClick={() => deleteDevice(item.merchandiseId)}>Удалить</strong>
                    <DeleteOutlined />
                </div>
            </div>
        </div>
