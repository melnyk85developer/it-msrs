...
import UpdateDevice from '../ContentModalsShops/updateDevice';
...
const DeviceItem: React.FC<PropsType> = React.memo(({
    ...
    const clickUpdateDevice = () => {
        dispatch(setDeviceDetailAC(deviceId))
        .then(() => setModalUpdateDeviceActive(true))
    }
    const deleteDivice = () => {
    ...
    return (
        <div className={classes.wrapItemDevice} style={click_deviceId === deviceId ? {borderColor: "#FFAC00"}: undefined} ref={dropdownRef}>
            ...
            <ModalWindows modalActive={modalUpdateDeviceActive} setModalActive={setModalUpdateDeviceActive}>
                <UpdateDevice 
                    shop={shop} 
                    device={device}
                    dispatch={dispatch}
                    page={page}
                    setModalUpdateDeviceActive={setModalUpdateDeviceActive}
                />
            </ModalWindows>
