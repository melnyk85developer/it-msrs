...
export const registrationAC = (name: string, surname: string, email: string, password: string, avatar?: any) => async (dispatch: AppDispatch) => {
    // console.log(name, surname, email, password, avatar)
    try{
        const data = await AuthAPI.registration(name, surname, email, password, avatar)
...
export const checkAuthAC = () => async (dispatch: AppDispatch) => {
    ...
    try{
        ...
        dispatch(authSlice.actions.userFetchingSuccess(response.data.user))
        dispatch(authSlice.actions.userIsAuth(true))
