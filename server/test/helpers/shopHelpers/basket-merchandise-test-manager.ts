...
export class ShopBasketMerchandiseTestManager {
    ...
    async getAllBasketsMerchandise(
        ...
        const { basketId, shopId, userId } = dataBasket
        // console.log('TEST - ⚙️ : - AuthTestManager - registration: req data', data)
        const response = await request(this.app.getHttpServer())
            .get(`${SETTINGS.RouterPath.backetd}`)
            .query({ basketId, shopId, userId })
    ...
    async getBasketMerchandiseById(
        ...
        const response = await request(this.app.getHttpServer())
            .get(`${SETTINGS.RouterPath.backetd}/${basketMerchandiseId}`)
    ...
    async createBasketMerchandise(
        ...
        if (expectedStatusCode === HTTP_STATUSES.CREATED_201) {
            ...
            expect(response.body.deviceId).toEqual(data.deviceId);
            expect(response.body.deviceName).toEqual(data.name);
