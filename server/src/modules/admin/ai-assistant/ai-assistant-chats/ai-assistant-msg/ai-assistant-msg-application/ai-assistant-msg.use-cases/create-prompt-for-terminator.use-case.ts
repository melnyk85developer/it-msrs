...
import { CreatePromptAiDto } from '../../ai-assistant-msg-dto/create-prompt-ai-assistant.dto';
import { CreateMessageAiAssistantCommand } from './create-msg-ai-assistant.use-case';
import { DomainException } from 'src/core/exceptions/domain-exceptions';
import { HTTP_STATUSES, INTERNAL_STATUS_CODE } from 'src/core/utils/utils';
import { AiAssistantMessageOneViewDto } from '../../ai-assistant-msg-api/viev-dto-msg/msg-one.view-dto';
...
export class CreatePromptForTerminatorUseCase
    ...
    async execute(command: CreatePromptForTerminatorCommand) {
        ...
        console.log('CreateMessageAiAssistantUseCase: - 👍🏻👍🏻👍🏻 dialogId ', dialogId)
        ...
        if (assistant && assistant.systemUserData && assistant.systemUserData.systemPrompts) {
            ...
            node = assistant.systemUserData.node
        }
        ...
        console.log('CreateMessageAiAssistantUseCase: - 👍🏻👍🏻👍🏻 ollPrompts ', ollPrompts.length)
        ...
        if (ollPrompts && Array.isArray(ollPrompts)) {
            const lastMessages = ollPrompts.slice(-20)
        ...
        console.log('CreateMessageAiAssistantUseCase: - 👍🏻👍🏻👍🏻 messages ', messages.length)


        // 🔥 ВОТ ТУТ ВСТАВКА

        // if (assistant.useEmbedding) {
        //     contextChunks = await embeddingService.search(prompt)

        //     messages.unshift({
        //         role: 'system',
        //         content: buildContext(contextChunks)
        //     })
        // }
        // console.log('CreateMessageAiAssistantUseCase: - 👍🏻👍🏻👍🏻 messages ', messages)

        if (provider1 === 'ollama' && model1) {
            console.log('CreateMessageAiAssistantUseCase: - 👍🏻👍🏻👍🏻 provider1 ', provider1)
            console.log('CreateMessageAiAssistantUseCase: - 👍🏻👍🏻👍🏻 model1 ', model1)
            try {
                const response = await this.sendToOllama(node, messages, model1);
        ...
        } else if (provider2 === 'ollama' && model2) {
        ...
        for (const node of this.nodes) {
            try {
                const response = await this.sendToOllama(node, messages, model2);
    ...
    // 🔹 OLLAMA
    private async sendToOllama(nodeUrl: string, prompt: { role: string, content: string }[], model?: string) {
        console.log('sendToOllama: - 👍🏻👍🏻👍🏻 prompt ', prompt)
        const { data } = await firstValueFrom(
            this.httpService.post(
