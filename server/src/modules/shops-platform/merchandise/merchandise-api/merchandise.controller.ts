...
export class MerchandiseController {
    ...
    @ApiOperation({ summary: 'Удалить один товар!' })
    @ApiResponse({ status: 204, type: Merchandise })
    @UseGuards(AuthAccessGuard)
    @Delete('/:merchandiseId')
    @HttpCode(HTTP_STATUSES.NO_CONTENT_204)
    async deleteMerchandiseController(@Param('merchandiseId') merchandiseId: string) {
    ...
    @Get()
    async getAllMerchandiseController(@Param('userId') userId: string, @Query() query: GetMerchandiseQueryParams): Promise<PaginatedViewDto<MerchandiseViewDto[]>> {
        // console.log('PhotoAlbumController: - getAllPhotoController userId', userId)
        return await this.merchandiseQueryRepository.getAllMerchandiseQueryRepository(query, userId)
    }
    @ApiOperation({ summary: 'Получить один товар!' })
    @ApiResponse({ status: 200, type: Merchandise })
    @Get('/:merchandiseId')
    async getMerchandiseByIdController(@Param('merchandiseId') merchandiseId: string) {
