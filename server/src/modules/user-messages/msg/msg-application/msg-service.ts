...
export class MessageService {
    ...
    async updateMessagesServices(dto: Omit<UpdateMessageDto, 'attachments'>, file?: [Multer.File]): Promise<string> {
        ...
        // console.log('updateMessagesServices: - 🤪🤪🤪 isUpdateMsg', isUpdateMsg)
        await this.messageRepository.save(msg);
        return msg._id.toString();
    }
    async updateReadServices(dto: UpdateMessageReadDomainDto): Promise<string> {
        // console.log('updateReadServices: - 👍🏻 msg ', msgId)
    ...
    async deleteAllMessagesServices(receiverId: string, userId: string, deleteOption: string): Promise<boolean> {
        ...
        if (messages && messages.length) {
            for (let i = 0; messages.length > i; i++) {
                ...
                // const msg = await this.messageRepository.findMessagesBySenderIdOrReceiverIdOrNotFoundFailRepository(senderId, receiverId);
                // const newMsg = messages[i].markMsgDeletedForUser({ msgId: messages[i].id, meta: newMeta });
        ...
        if (message && userId === message.senderId) {
            ...
            if (message.meta) {
                ...
                if (!alreadyDeleted && deleteOption === 'me') {
                    ...
                    return INTERNAL_STATUS_CODE.NOT_FOUND_MESSAGES_FOR_DELETE
                }
            } else {
                newMeta.push({ userId: userId, deletedAt: new Date().toISOString() })
            }
